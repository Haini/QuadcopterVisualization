function crashed_obstacles = hasCrashedObstacle( obstacles, obstacles_AABB, quadrotor_AABB, position, attitude ) %#codegen
...
%                     ground at hight 0.
persistent crashedObstacles;    % indicates obstacles that the quadrotor collided to
                                % clear with "clear hasCrashedObstacles"
...
for i = 1:num_obs_to_check
    ...
    if res > 0
        if crashedObstacles(obsid) ~= 1
            fprintf('hasCrashedObstacle: crash with obstacle %d (only first crash with that obstacle reported)!\n', int8(obsid));
            fprintf('  position (x,y,z): %.3f, %.3f, %.3f\n', x, y, z);
            fprintf('  obstacle: see obstacles(%d,:)\n', int8(obsid));
        end
...
if z < 0
    if crashedObstacles(num_obs+1) ~= 1
        fprintf('hasCrashedObstacle: crash with ground, better fly higher (only first crash with ground reported)!\n');
        fprintf('  position (x,y,z): %.3f, %.3f, %.3f\n', x, y, z);
        fprintf('  obstacle: see obstacles(%d,:)\n', int8(obsid));
    end
